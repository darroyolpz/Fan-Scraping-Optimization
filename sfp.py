...
for fileName in glob.glob('*.pdf'):
	...
	#---------------------------------------------------------------------------------------------------------------#
	# SFP scraping
	aWordStart = ['/s)Aprobado', '/s) 2018']
	aWordEnd = ['Chequeo total', 'W/(m']
	columns = ['Page', 'SFP1', 'SFP2']
	outter = extractFeatures(aWordStart, aWordEnd, 0, last_page, 1)
	df_sfp = pd.DataFrame(outter, columns = columns)
	df_sfp['Page'] = df_sfp['Page'].astype(int)

	# Function to assign the line number according to the page found
	list = []

	for page in df_sfp['Page'].values:
		value = df_units.loc[(df_units['Page start'].values < page) & (df_units['Page end'] > page), 'Line'].values[0]
		list.append(value)

	# Rename Page column for Line
	df_sfp.rename(columns = {'Page': 'Line'}, inplace=True)
	df_sfp['Line'] = list

	# Check the sfp_supply, extract sfp_extract, sfp_total and sfp_limit
	df_sfp['SFP Supply'], df_sfp['SFP Extract'], df_sfp['SFP Total'], df_sfp['SFP Limit'] = 0, 0, 0, 0

	# SFP Values
	for x in range(len(df_sfp['Line'])):
		if len(df_sfp['SFP2'].iloc[x]) < 9:
			df_sfp['SFP Limit'].iloc[x] = '230'
			sfp_val = df_sfp['SFP1'].iloc[x].replace('230', '')
			df_sfp['SFP Supply'].iloc[x] = sfp_val
			df_sfp['SFP Total'].iloc[x] = sfp_val
		else:
			df_sfp['SFP Extract'].iloc[x] = df_sfp['SFP2'].iloc[x][-3:]
			df_sfp['SFP Supply'].iloc[x] = df_sfp['SFP2'].iloc[x][-7:][:3]
			df_sfp['SFP Total'].iloc[x] = df_sfp['SFP1'].iloc[x][:len(df_sfp['SFP2'].iloc[x])-6]
			df_sfp['SFP Limit'].iloc[x] = df_sfp['SFP1'].iloc[x][len(df_sfp['SFP Total'].iloc[x]):]

	# Remove useless columns
	cols = ['SFP1', 'SFP2']
	df_sfp.drop(columns=cols, inplace=True)
	print('SFP:\n', df_sfp)

	name = 'SFP Raw.xlsx'
	writer = pd.ExcelWriter(name)
	df_sfp.to_excel(writer, index = False)
	writer.save()
	...
	df_sfp['Line'] = df_sfp['Line'].astype(str)
	df = pd.merge(df, df_sfp, on='Line')
